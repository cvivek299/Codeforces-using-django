...
def blog(request, blogId):
    ...
    allCommentList = Comment.objects.filter(blog=blog)
    ...
    if request.POST:
        if "blogVote" in request.POST:
            ...
            if popUpMessage is None:
                ...
                context = {
                    ...
                    'noOfComments': len(allCommentList),
                    'user': user,
                    'popUp': popUpMessage,
                }
                return HttpResponse(template.render(context, request))
        ...
        elif "replyButton" in request.POST:
            commentId, type = [x for x in request.POST.get("replyButton").split(',')]
            parentComment = Comment.objects.get(id=commentId)

            if type == "post":
                description = request.POST.get("commentBox")
                description = cleanMyField(description)
                myComment = Comment(description=description, user=user, blog=blog, parent=parentComment)
                myComment.save()
                BlogComment(blog=blog, comment=myComment).save()
                UserComment(user=user, comment=myComment).save()
                return HttpResponseRedirect("/blog/{}/".format(blogId))
